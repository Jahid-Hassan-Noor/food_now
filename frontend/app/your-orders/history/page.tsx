...
export default function OrderHistoryPage() {
  ...
  const loadHistory = React.useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await apiFetch<OrderHistoryResponse>("/your_orders/")
      ...
      const message = loadError instanceof Error ? loadError.message : "Unable to load order history."
      setError(message)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <>
      ...
      <div className="flex flex-1 flex-col gap-6 p-4 pt-0">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-2 max-w-2xl text-sm">
                  Review your completed orders and track spending trends over time.
                </p>
        ...
        {loading ? (
          <Card className="border-border/70 bg-card/95 shadow-sm">
            <CardContent className="py-8 text-sm text-muted-foreground">Loading order history...</CardContent>
          </Card>
        ) : null}
        ...
        {!loading && error ? (
          <Card className="border-red-300/70 bg-red-50/80 dark:border-red-900 dark:bg-red-950/30">
            <CardHeader>
              <CardTitle className="text-red-700 dark:text-red-300">Failed to load order history</CardTitle>
          ...
          <>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <MetricCard
                ...
                subtitle="All recorded history orders"
                icon={History}
              />
              <MetricCard
              ...
              <MetricCard
                ...
                subtitle="Amount in filtered view"
                icon={Wallet}
              />
              <MetricCard
                title="Items in View"
            ...
            <Card className="border-border/70 bg-card/95 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-foreground text-base">History Order List</CardTitle>
                <CardDescription className="text-muted-foreground text-sm">
                  Completed order records for your account.
                </CardDescription>
              </CardHeader>
              <CardContent className="overflow-x-auto">
                {!filteredOrders.length ? (
                  <p className="text-muted-foreground py-4 text-sm">No orders found for the selected filters.</p>
                ) : (
            ...
            <Card className="border-border/70 bg-card/95 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-foreground text-base">History Snapshot</CardTitle>
                <CardDescription className="text-muted-foreground text-sm">
                  Quick overview based on all history records.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-3 md:grid-cols-3">
                <div className="bg-muted/40 border-border rounded-xl border p-3">
                  ...
                  <p className="text-foreground mt-1 text-lg font-semibold">{formatNumber(summary.total_orders)}</p>
                </div>
                <div className="bg-muted/40 border-border rounded-xl border p-3">
                  <p className="text-muted-foreground text-xs uppercase tracking-wide">Total Items</p>
                  <p className="text-foreground mt-1 text-lg font-semibold">{formatNumber(summary.total_items)}</p>
                </div>
                <div className="bg-muted/40 border-border rounded-xl border p-3">
                  <p className="text-muted-foreground text-xs uppercase tracking-wide">Total Spend</p>
                  <p className="text-foreground mt-1 text-lg font-semibold">{formatCurrency(summary.total_amount)}</p>
                </div>
              </CardContent>
            </Card>
          </>
